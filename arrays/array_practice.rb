...
puts "If we delete 5, 6 and 7 from the array, we're left with [#{my_array.delete_if {|number| number > 4 && number < 8 }.join(",")}]"
puts "If we add 5 at the beginning of the array, we're left with [#{my_array.insert(0, 5).join(",")}]"
